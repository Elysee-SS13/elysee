...
// Krell: La condition ci-dessous permet de relancer le round apres que la shuttle arrive a centcom
	if(emergency_controller.has_evacuated() && emergency_controller.state == 3)
		emergency_controller.shuttle_evacuated()
	else if(next_location != waypoint_station)
